    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for old_position in self.legalPositions:
            new_position_distr = self.getPositionDistribution(self.setGhostPosition(gameState, old_position))
            for p, prob in new_position_distr.items():
                allPossible[p] = prob * self.beliefs[old_position] + allPossible[p]
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            allPossible, last_beliefs = util.Counter(), self.getBeliefDistribution()
            for location in self.legalPositions:
                distance = util.manhattanDistance(location, pacmanPosition)
                allPossible[location] = emissionModel[distance] * last_beliefs[location]
            if any(allPossible.values()):
                particles = []
                for i in range(self.numParticles): particles.append(util.sample(allPossible))
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        particles = []
        for last_position in self.particles:
            new_distance_position = self.getPositionDistribution(self.setGhostPosition(gameState, last_position))
            particles.append(util.sample(new_distance_position))
        self.particles = particles
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        counter, self.particles = 0, []
        random.shuffle(positions)
        while counter < self.numParticles:
            for p in positions:
                if self.numParticles > counter:
                    counter += 1
    ...
    def observeState(self, gameState):
        ...
        if any(allPossible.values()):
            ...
            for i in range(self.numParticles):
                temporary.append(util.sample(allPossible))
            self.particles = temporary
        else: self.initializeParticles()
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                new_distance_position = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), i, self.ghostAgents[i])
                newParticle[i] = util.sample(new_distance_position)
            "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        distribution = util.Counter()
        for element in self.particles: distribution[element] += 1
        distribution.normalize()
        return distribution
